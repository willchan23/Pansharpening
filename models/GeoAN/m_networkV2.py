...
class GeoAN(nn.Module):
    def __init__(self, window_sizes, n_geoab, c_in, c_geoan, r_expand=4, down_sample=4, num_heads=8):
        ...
        # self.input_down_Sequences = nn.Sequential(
        #     nn.Conv2d(c_in, c_in, kernel_size=3, stride=down_sample//2, padding=1),
        #     nn.Conv2d(c_in, c_in, kernel_size=3, stride=down_sample//2, padding=1)
        # )
...
if __name__ == '__main__':
    input_down_Sequences = nn.Sequential(
        nn.Conv2d(3, 3, kernel_size=3, stride=2, padding=1),
        nn.Conv2d(3, 3, kernel_size=3, stride=2, padding=1)
    )
    tensor1 = torch.zeros(3, 240, 360)
    tensor2 = input_down_Sequences(tensor1)
    print(tensor2.size())
